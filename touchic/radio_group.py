...
class ICRadioOption(ICBaseWidget):
    ...
    # radio color
    @property
    def radio_colors(self) -> [QtGui.QColor, QtGui.QColor]:
        return self._radio_border_color, self._radio_fill_color

    @radio_colors.setter
    def radio_colors(self, clrs: [QtGui.QColor, QtGui.QColor]) -> None:
    ...
    def paintEvent(self, e) -> None:
        ...
        if self._selector_type == ICRadioType.RadioType:
            ...
            rect = QtCore.QRectF(10, 10, tmp_hght - 20, tmp_hght - 20)
        ...
        if self._is_selected:
            if self._selector_type == ICRadioType.RadioType:
                ...
                rect = QtCore.QRectF(15, 15, tmp_hght - 30, tmp_hght - 30)
        ...
        if self.state in (ICWidgetState.VisibleEnabled, ICWidgetState.VisibleDisabled):
            ...
            # draw the text
            rect = QtCore.QRectF(tmp_hght, (tmp_hght - self._text_size)/2, tmp_wdth - tmp_hght, self._text_size + 5)
...
class ICRadioGroup(ICBaseWidget):
    ...
    def __init__(self, option_names: list[str], option_codes: list[int] = None,
                 selected: Union[str, int, list[int]] = None, selector_type: ICRadioType = ICRadioType.RadioType,
                 widget_id: int = 0, *args, **kwargs):
        ...
        if selector_type == ICRadioType.CheckBoxType:
            self._selected: list[bool] = len(option_names) * [False]
            ...
            self._selected: int = self._option_codes[0]
        ...
        for index, name in enumerate(self._option_names):
            ...
            # update row and column number to get a 4 column grid
            row_num = row_num if col_num < 3 else (row_num + 1)
            col_num = (col_num + 1) if col_num < 3 else 0

        self.setLayout(layout)

        # provide a size hint
        height_hint = 1.2 * row_num * ICDisplayConfig.RadioButtonHeight
        self.size_hint = (ICDisplayConfig.RadioButtonWidth, height_hint)
    ...
    def on_click(self, option_name: str, selected: bool) -> None:
        if self._selector_type == ICRadioType.RadioType:
            ...
            if self._option_codes[index] != self._selected:
                ...
                self.valueChanged.emit(self._selected)
        else:
            # multiple can be selected
            if option_name in self._option_names:
                index = self._option_names.index(option_name)
                self._selected[index] = selected
                self.valueChanged.emit(self._option_codes[index])
